...
class Trader:
    ...
    def run_osmium(self, state):
        ...
        # ===== REGIME DETECTION =====
        trend = self.osmium_ema - mid

        if abs(trend) > 1.5:
            fair_value = self.osmium_ema   # trending → follow market
        else:
            fair_value = 0.7 * self.osmium_anchor + 0.3 * self.osmium_ema  # mean revert

        # ===== ADAPTIVE VOL =====
        self.osmium_std = 0.9 * self.osmium_std + 0.1 * abs(mid - fair_value)

        z = (mid - fair_value) / max(1, self.osmium_std)
    ...
    def run_pepper(self, state: TradingState):
        ...
        if signal > self.buy_threshold:
            ...
            if qty > 0:
                ...
                # EARLY PHASE (cheap accumulation)
                if position < 25:
                    if signal > 0.8:
                        price = best_ask
                    else:
                ...
                # MID PHASE
                elif position < self.target_long:
                    if signal > 0.5:
                        price = best_ask
                    else:
                        price = best_bid + 1
