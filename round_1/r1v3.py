...
class Trader:
    ...
    def run_osmium(self, state):
        ...
        # ===== INVENTORY SKEW =====
        skew = int(position / 40)

        edge = max(1, spread // 4)

        bid_price = best_bid + edge - skew
        ask_price = best_ask - edge - skew
        ...
        # ===== MEAN REVERSION =====
        if z > 2:
            qty = min(12, position + self.osmium_limit)
            if qty > 0:
                orders.append(Order(product, best_bid, -qty))

        elif z < -2:
            qty = min(12, self.osmium_limit - position)
            if qty > 0:
                orders.append(Order(product, best_ask, qty))

        # ===== EXTREME =====
        if z > 3:
            qty = min(25 + int((z - 3) * 10), position + self.osmium_limit)
            orders.append(Order(product, best_bid, -qty))

        elif z < -3:
            qty = min(25 + int((abs(z) - 3) * 10), self.osmium_limit - position)
            orders.append(Order(product, best_ask, qty))
