...
class Trader:
    ...
    def run_osmium(self, state):
        ...
        # ===== FAIR VALUE =====
        fair_value = 0.9 * self.osmium_ema + 0.1 * self.osmium_anchor

        # ===== MICROPRICE EDGE (SAFE) =====
        ...
        if denom > 0:
            ...
            fair_value += 0.3 * (microprice - mid)

        # ===== Z-SCORE =====
        z = (mid - fair_value) / 2.0

        # ===== INVENTORY-AWARE SIZE =====
        ...
        # ===== EXTREME MOVES =====
        if z > 2.8:
            qty = min(25, position + 80)
            orders.append(Order(product, best_bid, -qty))

        elif z < -2.8:
            qty = min(25, 80 - position)
            orders.append(Order(product, best_ask, qty))

        # ===== INVENTORY PULLBACK (CRITICAL) =====
        if position > 30:
